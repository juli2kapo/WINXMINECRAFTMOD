 ...
 * STAGE 3 (final form): stage-2 cradle + large note-flag wings swept up and
 * back framing the egg (glowing tips), a tall spired crown and a bigger glowing
 * quaver finial.
 * =================================================================== */
...
// ---- 5b. STAGE 3 wings: a pair of large gold treble-clef wings framing the egg.
// Each wing: a thick spar sweeping out from the girdle and up past the crown,
// curling inward at the top into a clef scroll around a big glowstone jewel;
// three quaver-flag feathers hang outward from the spar, each with a glowing tip.
// The wings are swept back (azimuth behind the egg's sides) so the egg stays large.
function qb(a, b, c, t) { const it = 1 - t; return a * it * it + 2 * b * it * t + c * t * t; }
for (const sg of [1, -1]) {
  const phi0 = sg * (Math.PI / 2 + 0.15), back = 0.45 * sg;
  const yR = Y0 + 0.40 * E.h, rhoR = surfR(yR, phi0) + 0.5;
  // spar: root -> out -> up to the tip (quadratic Bezier in (rho, y)), then the clef curl
  const P0 = [rhoR, yR], P1 = [rhoR + 15, yR + 6], P2 = [rhoR + 9, Y0 + 1.14 * E.h];
  const spar = [], sparRY = [];
  for (let t = 0; t <= 1.0001; t += 0.025) {
    const rho = qb(P0[0], P1[0], P2[0], t), y = qb(P0[1], P1[1], P2[1], t);
    sparRY.push([rho, y, phi0 + back * t]);
    spar.push(azPt(phi0 + back * t, rho, y));
  }
  // clef curl at the top: turns inward (towards the egg) and down around a jewel
  const cr = 3.6, cRho = P2[0] - cr, cY = P2[1];
  for (let th = 0; th <= 1.55 * Math.PI; th += 0.1) {
    const rad = cr * (1 - 0.35 * th / (1.55 * Math.PI));
    spar.push(azPt(phi0 + back, cRho + rad * Math.cos(th), cY + rad * Math.sin(th)));
  }
  sweep(spar, 3.0, 1.6, G);
  const cj = azPt(phi0 + back, cRho, cY); ball(cj[0], cj[1], cj[2], 2.8, 'glowstone');
  // feathers hanging outward/down from the spar (quaver flags), glowing tips
  [[0.22, 7.0, -5.0, 2.6], [0.45, 8.0, -3.0, 2.5], [0.68, 7.0, -0.5, 2.3]].forEach(([s, dOut, dY, r]) => {
    const i = Math.round(s / 0.025), [rho, y, ph] = sparRY[i];
    const pts = [];
    for (let t = 0; t <= 1.0001; t += 0.05)
      pts.push(azPt(ph, rho + dOut * Math.sin(t * Math.PI / 2), y + dY * t - 2.5 * t * t + 2.0 * Math.sin(t * Math.PI)));
    sweep(pts.slice(0, 13), r, r * 0.85, G);
    sweep(pts.slice(12), r * 0.95, r * 0.8, 'glowstone');     // glowing wing tips
    const tp = pts[pts.length - 1];
    ball(tp[0], tp[1], tp[2], 2.6, 'glowstone');
  });
}
...
// ---- 6b. STAGE 3 tall crown: two tall side prongs curling outward (W silhouette
// around the finial) and a back spire, all tipped with glowstone.
function prong(phi, H, outC, rTip) {
  const y = Y0 + CAP_V * E.h - 1;
  const rho = surfR(y, phi) + 0.8;
  const pts = [];
  for (let t = 0; t <= 1.0001; t += 0.05) pts.push(azPt(phi, rho + outC * t * t, y + t * H));
  sweep(pts, 2.0, 1.1, G);
  const tp = pts[pts.length - 1];
  ball(tp[0], tp[1] + 0.8, tp[2], rTip, 'glowstone');
}
prong(Math.PI / 2, 11, 2.0, 2.4);
prong(-Math.PI / 2, 11, 2.0, 2.4);
prong(Math.PI, 14, 0.5, 2.0);
