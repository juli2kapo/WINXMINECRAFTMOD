 ...
 * STAGE 2: same egg + snowflake, plus a pair of ice-crystal shard wings
 * swept up and back from the egg's sides and a fuller icicle crown.
 * =================================================================== */
...
// ---- 5b. STAGE 2 wings: clusters of angular ice-crystal shards (faceted
...
for (const sg of [1, -1]) {
  ...
  const vR = 0.52, yR = Y0 + vR * E.h;
  const root = azPt(phi + sg * 0.12, surfR(yR, phi) + 0.5, yR);
  const SW = sg * 0.22; // swept slightly back
  // [elevation deg, length, radius, core, azimuth sweep extra, root dy]
  const S = [
    [42, 16, 3.4, 'white_wool', 0.00, 1],
    [14, 12, 2.8, 'white_wool', 0.05, -2],
    [70, 12, 2.8, 'white_wool', 0.10, 3],
    [-12, 7, 2.2, 'white_wool', 0.10, -5],
  ];
  for (const [el, len, rad, core, dp, dy] of S)
    shard([root[0], root[1] + dy, root[2]], dirAt(phi + SW + sg * dp, el * Math.PI / 180), len, rad, core, 'iron_block');
  ball(root[0], root[1], root[2], 2.4, G);               // gold socket
  const j = azPt(phi, surfR(yR, phi) + 1.6, yR); ball(j[0], j[1], j[2], 1.2, 'glowstone');
}

// ---- 6. crown (STAGE 2): fuller gold cap, 8 leaves, a ring of icicle spikes
...
for (let k = 0; k < 6; k++) {                 // icicle spikes fanning up and out from the cap
  ...
  shard(b, dirAt(phi, (50 + 10 * (1 - side)) * Math.PI / 180), 8 + 3 * side, 2.2, 'white_wool', 'iron_block');
}
shard([CX, Y1 - 1, CZ], [0, 1, 0], 13, 3.0, 'white_wool', 'iron_block');   // central crystal spire
ball(CX, Y1 + 0.5, CZ, 2.0, G);
