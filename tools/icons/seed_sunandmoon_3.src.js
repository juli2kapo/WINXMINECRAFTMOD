...
// ---- 6. STAGE 3 crown: tall radiant sun crown — gold cap, a ring of long
//         tapered sun rays (tallest at the back/sides, short over the emblem
//         window), glowstone jewels between them, and a big crescent-moon
//         finial on a spire cradling a glowing sun.
const CAP_V = 0.87;
...
for (let k = 0; k < RAYS; k++) {
  ...
  const L = front ? 3.5 : (k % 2 ? 6.5 : 9.5);      // alternating long / short rays, taller than stage 2
  const t = azPt(phi, surfR(yb, phi) + 1.2 + L * 0.35, yb + L * 0.95);
  sweep([b, t], 1.4, 0.4, G);
  if (!front && k % 2 === 0) ball(t[0], t[1], t[2], 1.1, 'glowstone');   // glowing ray tips
}
for (let k = 0; k < 4; k++) {
  const a = k * Math.PI / 2 + Math.PI / 4;
  const y = Y0 + CAP_V * E.h;
  const p = azPt(a, surfR(y, a) + 2.1, y);
  ball(p[0], p[1], p[2], 1.5, 'glowstone');
}
function uvw(u, y, w) { return [CX + u * Math.cos(VIEW) - w * Math.sin(VIEW), y, CZ + u * Math.sin(VIEW) + w * Math.cos(VIEW)]; }
function fin(u, y, w, m) { const p = uvw(u, y, w); block(Math.round(p[0]), Math.round(p[1]), Math.round(p[2]), m); }
// spire: tapered gold column with a glowing knot
sweep([[CX, Y1, CZ], [CX, Y1 + 3, CZ]], 2.4, 1.6, G);
ball(CX, Y1 + 2, CZ, 1.9, 'glowstone');
// crescent finial (bigger than stage 2), opening to the right, holding a glowing sun
const FB = Y1 + 3, FR = 6.6, FCY = FB + FR;
for (let dy = 0; dy <= 2 * FR + 1; dy += 0.5) for (let u = -FR - 1; u <= FR + 1; u += 0.5) for (let w = -1.5; w <= 1.5; w += 0.5) {
  ...
  const ci = du - 3.2, cv = dv - 0.7;
  const inI = ci * ci + cv * cv <= 5.0 * 5.0;
  if (inO && !inI && Math.abs(w) <= 1.2) fin(u, yy, w, G);
}
{ const s = uvw(3.0, FCY + 0.5, 0); ball(s[0], s[1], s[2], 2.3, 'glowstone'); }

// ---- 7. STAGE 3 wings: large radiant sunburst wings framing the egg, swept up
//         and back. Each wing: a fan of long tapered rays (gold, glowstone-tipped,
//         alternating with glowstone rays) rising from a hub at the egg's
//         shoulder, bounded by a gold crescent-moon arc sweeping around the fan's
//         outer edge, with a glowing star at the crescent's upper horn.
function star(c, r, m) {                             // 4-point star in the camera plane
  ball(c[0], c[1], c[2], r * 0.55, m);
  for (const [du, dy] of [[1, 0], [-1, 0], [0, 1], [0, -1]]) {
    const e = [c[0] + du * r * 1.6 * Math.cos(VIEW), c[1] + dy * r * 1.6, c[2] + du * r * 1.6 * Math.sin(VIEW)];
    sweep([c, e], r * 0.45, 0.3, m);
  }
}
function wing(side) {
  const vh = 0.56, yh = Y0 + vh * E.h;
  const uh = side * (prof(yh) - 1.5), wh = 3.5;     // hub tucked behind the emblem slab
  const N = 5;
  for (let i = 0; i < N; i++) {
    const el = (24 + i * 19) * Math.PI / 180;         // 24..100 deg above horizontal, well-separated rays
    const L = [13, 19, 22, 21, 16][i];
    const glow = i % 2 === 1;                          // alternate glowing rays
    const pts = [];
    for (let t = 0; t <= 1.0001; t += 0.1) {
      const d = L * t;
      // gentle upward/outward curve and backward sweep
      const bend = 0.18 * d * t;
      pts.push(uvw(uh + side * (d * Math.cos(el) - bend * Math.sin(el) * 0.3), yh + d * Math.sin(el) + bend * 0.6, wh + 3.0 * t));
    }
    sweep(pts, 2.7, 0.6, glow ? 'glowstone' : G);     // radiant: glowing rays alternate with gold rays
    const p = pts[pts.length - 1]; ball(p[0], p[1], p[2], glow ? 1.2 : 1.5, 'glowstone');
  }
  // hub: gold sun boss with a glowing centre
  const hub = uvw(uh, yh, wh);
  ball(hub[0], hub[1], hub[2], 3.0, G);
  const j = uvw(uh + side * 1.2, yh + 0.8, wh - 2.2); ball(j[0], j[1], j[2], 1.9, 'glowstone');
}
wing(1); wing(-1);
